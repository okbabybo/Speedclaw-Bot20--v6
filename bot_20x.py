...
LEVER = 20
RISK_PCT = 0.10
MIN_BAL = 10           # v6.0：3→10，低于10U不交易（避免交易所强平）
...
SL_AUTO_ENABLED = True         # 总开关：False时仅预警不平
...
def get_signal(symbol):
    ...
    if long_score >= (SCORE_THRESH_NORMAL if not STRONG_TREND_MODE else SCORE_THRESH_TRENDING):
    ...
    if short_score >= (SCORE_THRESH_NORMAL if not short_ready else SCORE_THRESH_TRENDING):
...
def main():
    log("="*60)
    log("20x 精准信号v6.0 | 自动止损+信号优化+凭证env")
    log("="*60)
